...
- **Finer y-axis gridlines when a year is isolated** — isolating a single year (double-clicking its legend entry, or manually deselecting every other year) now shows the y-axis at roughly 5x finer resolution. The newly-added in-between gridlines are unlabeled and lighter than the original step lines, so the extra detail doesn't compete with them.
...
### Fixed
- **Hourly refresh never actually re-fetching** — the client-side CSV cache in `lakeDataService.ts` never expired, so `useAppState`'s hourly poll silently kept re-serving the very first fetch for a lake instead of pulling fresh data. Cache entries now expire after 55 minutes, just under the poll interval, so the hourly refresh reaches the network again.
