...
export default async () => {
  ...
  await Promise.all(
    LAKE_IDS.map(async lake => {
      const url = `https://waterdatafortexas.org/reservoirs/individual/${lake}.csv`
      try {
        const res = await fetch(url)
        if (!res.ok) return
        const text = await res.text()
        await store.set(lake, trimHistoricalCSV(text), { metadata: { fetchedAt } })
      } catch {
        // skip this lake this run; the next scheduled run will retry
      }
    })
  )
}
